  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      if (canViewAll) {
        ...
        const siteIds = assignments.map((a: { siteId: string }) => a.siteId);

        if (siteIds.length === 0) {
          return reply.send(
            successResponse([], 'No sites assigned', {
              page: query.page,
              limit: query.limit,
              total: 0,
              totalPages: 0,
      ...
      return reply.send(
        successResponse(siteList, 'Sites retrieved successfully', {
          page: query.page,
          limit: query.limit,
          total: totalSites,
  ...
  fastify.delete(
    ...
    async (request, reply) => {
        ...
        if (!cascade) {
          return reply
            ...
            .send(
              errorResponse(
                'CONFLICT',
                `Cannot delete site "${existing.name}" because ${reportCount} report(s) are linked to it. Pass cascade=true to force delete the site and its reports.`
              )
            );
        }
      }
      ...
      await db.transaction(async (tx: any) => {
        ...
        const reportIds = siteReports.map((r: any) => r.id);
