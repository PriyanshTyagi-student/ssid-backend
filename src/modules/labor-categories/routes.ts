...
import { AuditAction, LaborCategoryType } from '../../config/constants.js';
...
export const laborCategoryRoutes: FastifyPluginAsync = async (fastify) => {
    ...
    async (_request, reply) => {
      ...
      const classifications = await db
        .select({
          ...
          total: sql<number>`count(${laborCategories.id})::int`,
          activeCount: sql<number>`count(${laborCategories.id}) filter (where ${laborCategories.isActive} = true)::int`,
        })
  ...
  fastify.put(
    ...
    async (request, reply) => {
      ...
      const body = request.body as { name?: string; newCode?: string; description?: string };
      ...
      const [existing] = await db
        ...
        .where(
          or(
            eq(laborClassifications.code, code),
            eq(laborClassifications.code, code.toLowerCase()),
            ilike(laborClassifications.code, code),
            ilike(laborClassifications.name, code)
          )
        )
        .limit(1);
      ...
      const updateData: Partial<typeof laborClassifications.$inferInsert> = {
        updatedAt: new Date(),
      };
    ...
    async (request, reply) => {
      ...
      if (!classification) {
        ...
        const [orphanedCount] = await db
          ...
          .where(
            or(
              eq(laborCategories.categoryType, code),
              eq(laborCategories.categoryType, code.toLowerCase()),
              ilike(laborCategories.categoryType, code)
            )
          );
        ...
        if (oCount > 0) {
          if (cascade) {
            await db.delete(laborCategories).where(
              or(
                eq(laborCategories.categoryType, code),
                eq(laborCategories.categoryType, code.toLowerCase()),
                ilike(laborCategories.categoryType, code)
      ...
      const [categoryCount] = await db
        ...
        .where(
          or(
            eq(laborCategories.categoryType, targetCode),
            ilike(laborCategories.categoryType, targetCode),
            eq(laborCategories.categoryType, code),
            ilike(laborCategories.categoryType, code)
          )
        );
      ...
      const [existing] = await db
        ...
        .where(
          and(
            ...
            body.parentId ? eq(laborCategories.parentId, body.parentId) : sql`${laborCategories.parentId} IS NULL`,
      ...
      const [duplicate] = await db
        ...
        .where(
          and(
            ...
            targetParentId ? eq(laborCategories.parentId, targetParentId) : sql`${laborCategories.parentId} IS NULL`,
