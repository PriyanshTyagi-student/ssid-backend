 ...
 */
export const ALL_VALID_PERMISSIONS = new Set<string>(
  PERMISSION_REGISTRY.flatMap((group) => group.permissions.map((p) => p.id))
);
...
export function isValidPermission(permission: string): boolean {
  ...
  if (ALL_VALID_PERMISSIONS.has(permission)) return true;
 ...
 */
export function hasPermission(userPermissions: string[] | undefined | null, required: string): boolean {
  ...
  if (mod === 'labor' || mod === 'labor_categories') {
    ...
    if (userPermissions.includes(`${altMod}.${action}`)) return true;
    if (userPermissions.includes(`${altMod}.manage`)) return true;
    if (userPermissions.includes(`${altMod}.*`)) return true;
  }
 ...
 */
export function hasAnyPermission(userPermissions: string[] | undefined | null, required: string[]): boolean {
 ...
 */
export function hasAllPermissions(userPermissions: string[] | undefined | null, required: string[]): boolean {
