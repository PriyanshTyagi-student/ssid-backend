...
export function requirePermission(...permissions) {
    return async (request, reply) => {
        ...
        if (!hasAnyPermission(userPermissions, permissions)) {
            return reply.status(403).send(errorResponse('FORBIDDEN', `Access forbidden. Required permission: ${permissions.join(' or ')}`));
        }
    };
}
/**
...
export function requireAllPermissions(...permissions) {
    return async (request, reply) => {
        ...
        if (missing.length > 0) {
            return reply.status(403).send(errorResponse('FORBIDDEN', `Access forbidden. Missing required permissions: ${missing.join(', ')}`));
        }
    };
}
/**
 ...
 */
export async function verifySiteAccess(userId, userRole, siteId, userPermissions = []) {
