    ...
    fastify.get('/', {
        ...
        if (canViewAll) {
            ...
            if (projectIds.length === 0) {
                return reply.send(successResponse([], 'No projects assigned', {
                    page: query.page,
                    limit: query.limit,
                    total: 0,
                    totalPages: 0,
        ...
        return reply.send(successResponse(projectList, 'Projects retrieved successfully', {
            page: query.page,
            limit: query.limit,
            total: totalProjects,
    ...
    fastify.delete('/:id', {
        ...
        if (reportCount > 0) {
            if (!cascade) {
                return reply
                    .status(409)
                    .send(errorResponse('CONFLICT', `Cannot delete project "${existing.name}" because ${reportCount} report(s) are associated with it. Pass cascade=true to force delete the project and all its reports.`));
            }
        }
