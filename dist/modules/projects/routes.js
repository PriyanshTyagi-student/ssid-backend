...
export const projectRoutes = async (fastify) => {
    ...
    fastify.delete('/:id', {
        ...
        if (Number(linkedReports.count) > 0) {
            return reply
                .status(409)
                .send(errorResponse('CONFLICT', `Cannot delete project "${existing.name}" because ${linkedReports.count} report(s) are associated with it. Please archive the project instead.`));
        }
