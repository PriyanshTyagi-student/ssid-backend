...
function validateAndNormalizeSections(sections, reportType) {
    ...
    for (const sec of sections) {
        ...
        for (const entry of sec.entries) {
            ...
            if (!data)
                continue;
            const isLaborEntry = reportType === 'labor' ||
...
async function validateLaborCategories(db, sections, isNewReport) {
    ...
    for (const sec of sections) {
        ...
        for (const entry of sec.entries) {
            ...
            if (data?.categoryId)
                ...
                categoryIds.add(data.classificationId);
        }
    }
...
export class ReportService {
     ...
     */
    static async listReports(query, user) {
        const db = getDb();
        ...
        if (query.reportType)
            conditions.push(eq(reports.reportType, query.reportType));
        if (query.projectId)
            conditions.push(eq(reports.projectId, query.projectId));
        if (query.siteId)
            conditions.push(eq(reports.siteId, query.siteId));
        if (query.status)
            conditions.push(eq(reports.status, query.status));
        if (query.createdBy)
            conditions.push(eq(reports.createdBy, query.createdBy));
        if (query.dateFrom)
            conditions.push(gte(reports.reportDate, query.dateFrom));
        if (query.dateTo)
            conditions.push(lte(reports.reportDate, query.dateTo));
     ...
     */
    static async getReportById(id, user) {
        const db = getDb();
        ...
        const fullSections = await Promise.all(sections.map(async (sec) => {
    ...
    static async createReport(input, user, ipAddress, userAgent) {
        ...
        const [todayCountRes] = await db
            .select({ count: count() })
            .from(reports)
            .where(and(eq(reports.reportType, input.reportType), eq(reports.reportDate, input.reportDate)));
        const sequence = Number(todayCountRes.count) + 1;
        const reportNumber = generateReportNumber(input.reportType, today, sequence);
        ...
        const createdReport = await db.transaction(async (tx) => {
            ...
            for (const sec of input.sections) {
                ...
                if (sec.entries && sec.entries.length > 0) {
                    const entryValues = sec.entries.map((entry) => ({
     ...
     */
    static async reviewReport(reportId, reviewerId, ipAddress, userAgent) {
        const db = getDb();
        const [report] = await db.select().from(reports).where(eq(reports.id, reportId)).limit(1);
        if (!report) {
            throw new Error('Report not found');
     ...
     */
    static async approveReport(reportId, reviewerId, ipAddress, userAgent) {
        const db = getDb();
        const [report] = await db.select().from(reports).where(eq(reports.id, reportId)).limit(1);
        if (!report) {
            throw new Error('Report not found');
     ...
     */
    static async rejectReport(reportId, reviewerId, rejectionReason, ipAddress, userAgent) {
        const db = getDb();
        const [report] = await db.select().from(reports).where(eq(reports.id, reportId)).limit(1);
        if (!report) {
            throw new Error('Report not found');
     ...
     */
    static async exportReports(query, user, ipAddress, userAgent) {
