...
export const assignmentRoutes = async (fastify) => {
    ...
    fastify.get('/users', {
        ...
        const allSiteAssignments = await db
            ...
            .from(userSiteAssignments)
            .innerJoin(sites, eq(userSiteAssignments.siteId, sites.id));
        const userMatrix = fieldUsers.map((u) => {
            const userProjects = allProjectAssignments.filter((pa) => pa.userId === u.id);
            const userSites = allSiteAssignments.filter((sa) => sa.userId === u.id);
            return {
                ...u,
                projectCount: userProjects.length,
                siteCount: userSites.length,
                assignedProjects: userProjects.map((p) => ({ id: p.projectId, name: p.projectName })),
                assignedSites: userSites.map((s) => ({ id: s.siteId, name: s.siteName, projectId: s.projectId })),
            };
        });
        ...
        if (siteIds.length > 0) {
            ...
            for (const s of foundSites) {
                if (!projectIds.includes(s.projectId)) {
                    return reply.status(400).send(errorResponse('SITE_PROJECT_MISMATCH', `Site ${s.id} belongs to project ${s.projectId}, which is not in the assigned projects list. Please include the project.`));
                }
            }
        }
